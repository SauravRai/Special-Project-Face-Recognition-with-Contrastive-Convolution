...
class network_9layers(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc2(x)
        return f, out,  feature  #output,feature 
...
class network_4layers(nn.Module):   #Added by Darshan on 14-07-18
    def __init__(self, num_classes = 10000):
        ...
        self.features = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True), 
            mfm(128, 192, 4, 1, 0),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True), #192x5x5
            )  
        self.fc1 = mfm(5*5*192, 256, type = 0)  
        #self.drop1 = nn.Dropout2d(p = 0.5) #added by bala on 25-7-2018
        self.fc2 = nn.Linear(256, num_classes)

    def forward(self, x):
        x = self.features(x)
        f = x   #before fc1 layer o/p: 192x5x5
      
        x = x.view(x.size(0), -1)
        x = self.fc1(x)
        
        feature = x   #lightcnn feature
        
        x = F.dropout(x, training = self.training)
        
        out = self.fc2(x)
        
        return  f, out, feature
